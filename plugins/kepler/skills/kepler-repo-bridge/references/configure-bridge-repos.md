...
1. Locate the Hub by walking upward to `kepler.yaml`.
2. Read the Hub's applicable `AGENTS.md` completely.
3. Read `kepler.yaml`, `hub/repositories.yaml`,
...
3. If it is absent and clone authorization is within the user's request, run
   `bin/kepler repo plan` with the verified facts, inspect the plan, then run
   `bin/kepler repo onboard` under the declared workload root.
