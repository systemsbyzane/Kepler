...
patch_notes = load_module(
    "kepler_patch_notes", SKILL_ROOT / "scripts" / "patch_notes.py"
)
upgrade_planner = load_module(
    "kepler_upgrade_planner", SKILL_ROOT / "scripts" / "upgrade_planner.py"
)
...
class UpgradePlannerTests(unittest.TestCase):
    def create_marketplace(
        ...
        plugin_root = root / "plugins" / "kepler"
        ...
        (plugin_root / ".codex-plugin" / "plugin.json").write_text(
            json.dumps({"name": "kepler", "version": target}),
            encoding="utf-8",
        )
        (plugin_root / "releases.json").write_text(
            json.dumps(
                {
                    "schema_version": "kepler-releases/v1",
                    "releases": [
                        {
                            "version": target,
        ...
        plugins = {
            "installed": [
                {
                    "pluginId": "kepler@kepler-team",
                    "name": "kepler",
                    "marketplaceName": "kepler-team",
                    "version": "1.0.0+codex.1",
                    "installed": True,
                    "enabled": True,
                }
    ...
    def test_local_marketplace_plan_skips_refresh_and_preserves_state(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            plan = upgrade_planner.build_plan(
                plugins, marketplaces, "kepler", "kepler-team"
            )
    ...
    def test_target_ledger_must_match_target_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            releases_path = root / "plugins" / "kepler" / "releases.json"
