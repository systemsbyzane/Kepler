---
name: kepler-upgrade
description: Plan, explain, apply, and verify a supported Kepler plugin upgrade without regenerating a Hub or changing attached repositories. Use when a user asks to upgrade or update Kepler, reinstall a newer Kepler plugin build, check whether an update is available, preserve existing Kepler state during an update, roll back an attempted plugin update, or asks what changed in Kepler, for Kepler patch notes, or for changes between Kepler versions.
---
...
Natural Kepler upgrade and patch-note intent triggers this skill;
...
`scripts/hub_compatibility.py` for `kepler.command.doctor.v1` or
`kepler.document.plugin-lifecycle.v1` before requiring Hub-local Doctor or
...
6. Re-run the planner, then reinstall the same logical plugin with
   `codex plugin add kepler@<marketplace> --json`. Do not remove it first,
