# Kepler plugin upgrade contract

## Outcomes

A safe upgrade:

- resolves one installed Kepler plugin and its configured marketplace;
...
| Generated Kepler Hub | User | Protected; never regenerated or migrated |
...
5. If a generated Hub is open, run the installed setup skill's
   `scripts/hub_compatibility.py` for `kepler.command.doctor.v1` before
   invoking Doctor. Capture `bin/kepler doctor --json` only for a compatible
...
```sh
codex plugin add kepler@<marketplace> --json
```
