...
class HubCompatibilityTest(unittest.TestCase):
    ...
    def test_legacy_hub_missing_setup_command_returns_migration_plan(self) -> None:
        with tempfile.TemporaryDirectory(prefix="kepler-compatibility-") as directory:
            hub = Path(directory) / "legacy-hub"
            (hub / "bin").mkdir(parents=True)
            entrypoint = hub / "bin" / "kepler"
            entrypoint.write_text(
                "#!/usr/bin/env python3\n"
                "print('bin/kepler doctor [--json]\\n"
                "bin/kepler status [--json]\\n"
                "bin/kepler route plan --workload NAME')\n",
                encoding="utf-8",
            )
            ...
            self.assertEqual(["kepler.command.setup-plan.v1"], [item["id"] for item in missing])
            self.assertEqual("stop_and_plan_migration", missing[0]["fallback"]["mode"])
            self.assertIn("lib/kepler/setup_store.rb", report["migration"]["managed_paths_to_compare"])
    ...
    def test_legacy_hub_missing_change_review_uses_bundled_reference_fallback(self) -> None:
        with tempfile.TemporaryDirectory(prefix="kepler-compatibility-") as directory:
            ...
            self.assertEqual(
                "skills/kepler-review/references/review-method.md",
    ...
    def test_target_contract_requires_a_precise_bundled_fallback(self) -> None:
        with tempfile.TemporaryDirectory(prefix="kepler-compatibility-") as directory:
            ...
            del contract["capabilities"]["kepler.document.change-review.v1"][
                "fallback"
            ]["reference"]
