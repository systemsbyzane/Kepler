...
This runbook is mandatory whenever an agent sets up a Kepler. Complete it in
...
5. Treat configured workload repositories, program payloads, ignored state,
   `kepler.yaml`, and `hub/repositories.yaml` as user-owned after generation.
...
python3 scripts/hub_compatibility.py \
  ...
  --require kepler.command.doctor.v1
```

Repository discovery and connection also require
`kepler.command.setup-plan.v1` and
`kepler.command.setup-connect.v1`. Bootstrap itself requires a declared Hub
...
Parse every Kepler coordination-layer JSON, YAML, and schema file. Do not treat
configured workload repository files, program payloads, or ignored runtime
state as Kepler source. Treat Doctor errors, test failures, de-branding
findings, invalid links, or coordination-layer parse failures as setup failure.
Doctor warnings must be reported with their exact no-fetch limitation.

Edit only stable topology in `kepler.yaml`. Never add credentials, live branch
...
`bin/kepler doctor --json` after project verification.
...
The deterministic local harness validates generation and coordination-layer behavior:
