...
- Choose the smallest lead Kepler skill from the requested outcome, not
  from the owning workload, repository, or bridge profile.
- Include the exact lead and currently applicable companion `$kepler-*`
...
- In the Hub, plan ownership and sequence from registry and routing evidence.
  ...
  the user also asks Kepler to have that owner investigate or proceed.
...
## Kepler plugin lifecycle

- Natural Kepler update, reinstall, version, rollback, preservation, or
  patch-note intent uses the installed `$kepler-upgrade` skill.
...
Use `bin/kepler doctor --json`, `status`, `route plan`, `repo plan`, and
...
Read `docs/architecture/coordination-layer.md` and
`docs/workflow.md` before changing the Hub itself.
