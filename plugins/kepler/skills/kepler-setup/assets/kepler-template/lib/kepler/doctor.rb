...
module Kepler
  class Doctor
    ...
    def run
      ...
      coordination, coordination_issues = inspect_coordination
      issues.concat(coordination_issues)
      ...
      {
        "schema_version" => "kepler.doctor/v1",
    ...
    def inspect_coordination
      ...
      plans = Dir.glob(File.join(@config.plan_dir, "*.yaml")).sort
      plans.each do |path|
        PlanStore.new(@config).load(File.basename(path, ".yaml"))
      rescue ValidationError => e
        issues << issue("error", "plan.invalid", Support.relative_path(@config.root, path), e.message)
      end
      [{ "architecture_map" => architecture, "plans" => plans.length }, issues]
    end
