...
module Kepler
  class Config
    ...
    def initialize(root:, path: nil)
      ...
      @path = File.expand_path(path || ENV.fetch("KEPLER_CONFIG", File.join(@root, "kepler.yaml")))
      raise ConfigurationError, "missing Hub configuration: #{@path}" unless File.file?(@path)

      @data = Support.load_data(@path)
      raise ConfigurationError, "kepler.yaml must contain a mapping" unless @data.is_a?(Hash)
    ...
    def architecture_map_path
      root_path(workspace.fetch("architecture_map", "hub/architecture-map.yaml"), label: "architecture map")
    end

    def plan_dir
      root_path(workspace.fetch("plans_root", "hub/plans"), label: "plans root")
    end

    def context_pack_dir
      root_path(workspace.fetch("context_packs_root", "hub/context-packs"), label: "context packs root")
    end

    def worker_result_dir
      root_path(workspace.fetch("worker_results_root", "hub/worker-results"), label: "worker results root")
    end

    def memory_dir
      root_path(workspace.fetch("memory_root", "hub/memory"), label: "memory root")
    end

    def dispatch_receipt_dir
    ...
    def repository_declarations
      ...
      unless value["api_version"] == "kepler.dev/v1alpha1" &&
             value["kind"] == "RepositoryDeclarations"
        raise ConfigurationError, "repository declarations must use kepler.dev/v1alpha1 RepositoryDeclarations"
      end
    ...
    def project_verifications
      ...
      unless value["api_version"] == "kepler.dev/v1alpha1" &&
        ...
        raise ConfigurationError, "project registry must use kepler.dev/v1alpha1 CodexProjectVerifications"
      end
    ...
    def mapping(name)
      ...
      raise ConfigurationError, "kepler.yaml #{name} must be a mapping" unless value.is_a?(Hash)
