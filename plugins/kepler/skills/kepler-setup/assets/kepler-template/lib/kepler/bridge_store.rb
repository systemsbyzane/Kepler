  ...
  class BridgeStore
    ...
    def plan(repository_id:, mode:, profile: nil)
      ...
      targets = case mode
                ...
                when "materialized"
                  [
                    ".kepler/bridge/AGENTS.md",
                    "AGENTS.override.md",
                    *Array(definition["required_docs"]).map { |path| File.join(".kepler/bridge", path) }
                  ]
      ...
      if record
        ...
        marker = "<!-- kepler-bridge:#{manifest.fetch('bridge_version')} -->"
    ...
    def install(repository_id:, mode:, profile: nil, acknowledge_repo_native: false)
      ...
      target = case mode
               ...
               when "materialized"
                 policy = File.join(root, ".kepler", "bridge", "AGENTS.md")
                 ...
                 template = render_template(
                   ...
                   ".",
                   mode: mode,
                   hub_root: ".kepler/bridge"
                 )
                 write_new(policy, template)
                 override = <<~MARKDOWN
                   # Kepler materialized bridge

                   Read `./AGENTS.md` completely, then read
                   `./.kepler/bridge/AGENTS.md`. Repository rules own commands,
               ...
               when "repo-native"
                 ...
                 marker = "<!-- kepler-bridge:#{plan_value.fetch('version')} -->"
    ...
    def record_integrity(record, repository_root:, mode:, profile:)
      ...
      begin
        ...
        if mode == "materialized" && !ignored?(repository_root, ".kepler/bridge/AGENTS.md")
