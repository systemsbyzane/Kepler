...
1. Classify intent and choose the smallest lead Kepler skill for the
   requested outcome: planning, review, CI/CD, platform, read-only,
   implementation, compliance, runtime-validation, or coordination.
2. Resolve workload and ownership from `kepler.yaml`, repo code, artifacts,
   GitHub, or authorized environment evidence.
3. Use `bin/kepler route plan --workload <id> --work-type <type>` and add
...
6. Require `bridge_handoff.status: verified` for a repository owner and include
   ...
   `$kepler-*` names in the child prompt.
...
own a live deployment while `$kepler-platform` remains the lead skill.
...
owns the patch, clone it under `patching/`, register it in `kepler.yaml`,
