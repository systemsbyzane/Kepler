...
BRIDGE_SKILL = ROOT.parent / "kepler-repo-bridge"
PLAN_SKILL = ROOT.parent / "kepler-plan"
REVIEW_SKILL = ROOT.parent / "kepler-review"
CI_SKILL = ROOT.parent / "kepler-ci"
PLATFORM_SKILL = ROOT.parent / "kepler-platform"
DB_SKILL = ROOT.parent / "kepler-db"
STIG_SKILL = ROOT.parent / "kepler-stig"
UPGRADE_SKILL = ROOT.parent / "kepler-upgrade"
COORDINATOR_SKILL = ROOT.parent / "kepler"
REQUIRED = (
...
def local_markdown_link_failures(root: Path) -> list[str]:
    ...
    for path in sorted(root.rglob("*.md")):
        if any(part in {".git", ".kepler-local", "__pycache__"} for part in path.parts):
            continue
...
def main() -> int:
    ...
    if SOURCE_REPOSITORY is not None:
        ...
        required_readme_links = (
            "plugins/kepler/skills/kepler-setup/references/setup-runbook.md",
            "plugins/kepler/skills/kepler-repo-bridge/references/configure-bridge-repos.md",
        )
    ...
    if "references/configure-bridge-repos.md" not in bridge_skill or "mandatory" not in bridge_skill.lower():
        failures.append("kepler-repo-bridge/SKILL.md must mandate configure-bridge-repos.md")
    ...
    focused_skills = (
        ...
        (
            UPGRADE_SKILL,
            ("natural kepler upgrade", "explicit invocation is optional"),
    ...
    composition_paths = (
        ...
        TEMPLATE / "AGENTS.md",
        TEMPLATE / "docs" / "workflows" / "thread-routing.md",
        TEMPLATE / "lib" / "kepler" / "route_planner.rb",
    )
    ...
    for trigger in ("ssh", "remote-host", "kind", "kubectl", "live deployments"):
        if trigger not in platform_description:
            failures.append(
                f"kepler-platform/SKILL.md description missing trigger: {trigger}"
            )
    ...
    for trigger in (
        ...
        if trigger not in database_description:
            failures.append(
                f"kepler-db/SKILL.md description missing trigger: {trigger}"
            )
