#!/usr/bin/env python3
"""Generate a portable Kepler from the bundled template."""
...
def main() -> int:
    ...
    template = Path(__file__).resolve().parent.parent / "assets" / "kepler-template"
    ...
    staging = Path(tempfile.mkdtemp(prefix=".kepler-setup-", dir=target.parent))
    try:
        shutil.copytree(template, staging, dirs_exist_ok=True)
        registry = staging / "kepler.yaml"
        escaped_root = json.dumps(str(target), ensure_ascii=False)[1:-1]
        atomic_text(
            registry,
            registry.read_text(encoding="utf-8").replace("__KEPLER_ROOT__", escaped_root),
        )
        ...
        (staging / "bin" / "kepler").chmod(0o755)
    ...
    result = {
        "schema_version": "kepler.setup/v1",
        "target": str(target),
        "generated": True,
        "git_initialized": not args.no_git,
        "merge_performed": False,
        "next": f"{target / 'bin' / 'kepler'} doctor --json",
    }
    if args.json:
        print(json.dumps(result, indent=2, sort_keys=True))
    else:
        print(f"Generated Kepler: {target}")
