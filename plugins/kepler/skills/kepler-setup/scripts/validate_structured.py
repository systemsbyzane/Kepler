...
def generated_boundaries(
    ...
    try:
        ...
        return (), ()
    if not isinstance(config, dict) or config.get("kind") != "KeplerRegistry":
        return (), ()
...
def main() -> int:
    ...
    report = {
        "schema_version": "kepler.structured-validation/v1",
        "root": str(root),
        "counts": counts,
        "failures": failures,
        "ok": not failures,
