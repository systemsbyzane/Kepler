...
def require_local_report(path: Path) -> None:
    if ".kepler-local" not in path.resolve().parts:
        raise InventoryError("JSON report must be written under .kepler-local")
...
def candidate_files(root: Path) -> list[str]:
    ...
    for path in root.rglob("*"):
        ...
        if (
            ...
            or any(
                part in {".git", ".kepler-local", "__pycache__"}
...
def canonical_source_paths(paths: list[str]) -> dict[str, str]:
    """Derive neutral aliases for source-specific coordination-layer filenames."""
    ...
    if len(library_roots) == 1:
        ...
        for path in paths:
            ...
            if path.startswith(prefix):
                aliases[path] = f"lib/@coordination-layer/{path[len(prefix):]}"
...
def build_report(
    *,
    ...
    asset_prefix = "skills/kepler-setup/assets/kepler-template/"
