#!/usr/bin/env python3
"""Preview or apply a deterministic first-time Kepler bootstrap."""
...
TEMPLATE = SKILL_ROOT / "assets" / "kepler-template"
...
def normalize_target(requested: Path) -> Path:
    ...
    try:
        ...
        if not source_relative.parts or source_relative.parts[0] != ".kepler-local":
            raise BootstrapFailure(
                "target",
                f"target must be outside the Kepler plugin source: {target}",
                exit_code=2,
            )
    return target
...
def expected_managed_bytes(source: Path, target: Path) -> bytes:
    ...
    if source.relative_to(TEMPLATE) == Path("kepler.yaml"):
        ...
        content = content.replace(b"__KEPLER_ROOT__", escaped_root)
    return content
...
def recognize_generated_target(target: Path, ruby: str) -> None:
    ...
    if missing:
        raise BootstrapFailure(
            "target-recognition",
            "non-empty target is not a complete generated Kepler; "
    ...
    registry = parse_registry(target / "kepler.yaml", ruby)
    ...
    coordination = projects.get("kepler") if isinstance(projects, dict) else None
    coordination_path = (
    ...
    if (
        ...
        raise BootstrapFailure(
            "target-recognition",
            "non-empty target is not a generated Kepler for the exact normalized "
            f"path {target}",
            exit_code=2,
        )
...
def preserved_hub_compatibility(
    ...
    *,
    ...
    if any(not path.is_file() for path in required_hub_paths):
        return None
    requirements = ["kepler.command.doctor.v1"]
...
def base_report(target: Path, mode: str, preflight_report: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "kepler.bootstrap/v1",
        "target": str(target),
        "mode": mode,
        "status": "pending",
        "generated": False,
...
def bootstrap(
    ...
    *,
    ...
    if setup.get("generated") is not True or setup.get("target") != str(target):
        raise BootstrapFailure(
            "generation", "setup_kepler.py did not confirm the exact target"
        )
...
def human_output(report: dict[str, Any]) -> str:
    ...
    if report["status"] == "preview":
        lines.extend(
            [
                "Preview: a new Kepler would be generated at this exact path.",
    ...
    action = (
        "Existing Kepler validated."
        if report["status"].startswith("validated")
        else "Kepler generated and validated."
    )
    lines.extend(
        [
            action,
