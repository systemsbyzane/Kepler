#!/usr/bin/env python3
"""Run deterministic local acceptance probes against a fresh generated Kepler."""
...
INSTALLED_RUNTIME_ACCEPTANCE_NAMES = (
...
def runtime_acceptance_placeholders() -> list[dict[str, str]]:
    placeholders = [
        ...
        for name in INSTALLED_RUNTIME_ACCEPTANCE_NAMES
    ]
    placeholders.append(
        {
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        "schema_version": "kepler.acceptance/v1",
        ...
        with tempfile.TemporaryDirectory(prefix="kepler-acceptance-") as directory:
            ...
            compatibility = json_command(
                [
                    ...
                    "--require",
                    "kepler.command.setup-connect.v1",
                    "--require",
            ...
            probe(
                ...
                and "natural kepler update" in hub_instructions
            ...
            nested_validator_root = root / ".kepler-local" / "nested-validator-root"
            ...
            tests = run(["ruby", "-Ilib", "tests/kepler_test.rb"], cwd=hub)
            probe(report, "generated_ruby_tests", "0 failures" in tests.stdout, tests.stdout.strip())

            initial_doctor = json_command([str(hub / "bin/kepler"), "doctor", "--json"], cwd=hub)
            probe(
                report,
                "fresh_doctor",
            ...
            probe(
                ...
                "lead Kepler skill" in child_prompt_text
            ...
            architecture = {
                ...
                "workspaces": {
                    "product": {
                        "codex_project": "synthetic-reference",
                        "repository": "synthetic-reference",
                        "domains": {"backend": {"paths": ["lib"]}},
                    }
                },
            }
            architecture_source = root / "architecture-map.yaml"
            architecture_source.write_text(json.dumps(architecture, indent=2) + "\n", encoding="utf-8")
            run([str(hub / "bin/kepler"), "architecture", "confirm", str(architecture_source)], cwd=hub)

            plan = {
                "api_version": "kepler.dev/v1",
                "kind": "Plan",
                "metadata": {"id": "synthetic-change", "revision": 1},
                "objective": "Preserve the synthetic contract",
                "status": {"state": "draft"},
                "units": [
                    {
                        "id": "backend",
            ...
            plan_source = root / "plan.yaml"
            plan_source.write_text(json.dumps(plan, indent=2) + "\n", encoding="utf-8")
            run([str(hub / "bin/kepler"), "plan", "apply", str(plan_source)], cwd=hub)
            duplicate = run(
                [str(hub / "bin/kepler"), "plan", "apply", str(plan_source)],
                cwd=hub,
                expect=1,
            )
            probe(
                report,
                "plan_non_monotonic_revision_refusal",
                "revision must increase" in duplicate.stderr,
            ...
            probe(
                report,
                "dispatch_stale_revision_gate",
                "stale Plan revision" in gate.stderr,
                gate.stderr.strip(),
            )
            ...
            probe(
                report,
                "dispatch_exact_verified_project_and_path",
                prepared_unit["runtime_project_id"]
                == projects["projects"]["synthetic-reference"]["runtime_project_id"]
                and Path(prepared_unit["project_path"]).resolve()
                == (hub / "development/synthetic-reference").resolve(),
                {
                    "runtime_project_id": prepared_unit["runtime_project_id"],
                    "project_path": prepared_unit["project_path"],
                },
            )

            doctor = json_command([str(hub / "bin/kepler"), "doctor", "--json"], cwd=hub)
            probe(
                report,
            ...
            probe(
                ...
                "Kepler validation passed." in managed_validation.stdout
