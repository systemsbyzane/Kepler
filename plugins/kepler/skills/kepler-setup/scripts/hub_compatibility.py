#!/usr/bin/env python3
"""Check a preserved generated Hub against requested Kepler capabilities."""
...
SCHEMA_VERSION = "kepler.hub-compatibility/v1"
RESULT_SCHEMA_VERSION = "kepler.hub-compatibility-result/v1"
SKILL_ROOT = Path(__file__).resolve().parent.parent
DEFAULT_TARGET_CONTRACT = (
    SKILL_ROOT / "assets" / "kepler-template" / "hub" / "compatibility.json"
)
COMMAND_TIMEOUT = 30
CONTRACT_SCHEMA = "hub/schemas/hub-compatibility.schema.json"
CAPABILITY_ID = re.compile(
    r"^kepler\.(command|document)\.[a-z0-9-]+\.v[0-9]+$"
)
FALLBACK_MODES = {
    "bundled_reference",
...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    if value.get("product") != "kepler":
        raise ContractError("contract product must be kepler")
    ...
    for capability_id, capability in capabilities.items():
        ...
        if match is None:
            raise ContractError("capability IDs must use the versioned kepler command or document namespace")
...
def command_help(hub_root: Path) -> tuple[str | None, str | None]:
    executable = hub_root / "bin" / "kepler"
...
def compatibility_result(
    *,
    ...
    hub_identity = {
        "product": hub_contract.get("product") if hub_contract else "kepler",
