#!/usr/bin/env python3
"""Compare mandatory Kepler behavior using neutral mappings and functional probes."""
...
def neutral_string(value: str) -> str:
    ...
    output = output.replace("kepler.organization.dev", "kepler.dev")
...
def copy_source_test_snapshot(
    ...
    """Copy source coordination-layer files without local Hub payloads."""
    ...
    if not names:
        raise ValueError("source Hub has no safe coordination-layer files")
    for name in names:
        relative = Path(name)
...
def atomic_text(path: Path, content: str) -> None:
    ...
    descriptor, temporary = tempfile.mkstemp(prefix=".kepler-", dir=path.parent)
    try:
...
def require_ignored_output(path: Path) -> None:
    if ".kepler-local" not in path.resolve().parts:
        raise ValueError(f"comparison output must be under .kepler-local: {path}")
...
def compare_schema(source: Path, candidate: Path) -> tuple[bool, dict[str, Any]]:
    ...
    for path, value in left["constraints"].items():
        ...
        repository_store = candidate_root / "lib" / "kepler" / "repository_store.rb"
        store_text = (
        ...
        if (
            candidate.name == "kepler.schema.json"
...
def file_inventory(root: Path) -> list[str]:
    return [
        ...
        if path.is_file()
        and not any(
            part in {".git", ".kepler-local", "__pycache__"} for part in path.parts
        )
    ]
...
def generated_template_alignment(
    ...
    for source in sorted(path for path in candidate.rglob("*") if path.is_file()):
        ...
        if relative == Path("kepler.yaml"):
            escaped_root = json.dumps(
                str(generated_hub.resolve()), ensure_ascii=False
            )[1:-1].encode("utf-8")
            expected = expected.replace(b"__KEPLER_ROOT__", escaped_root)
...
def compare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    setup_scripts = plugin / "skills" / "kepler-setup" / "scripts"
    ...
    records.append(
        surface(
            "legacy_task_schema_semantics",
            status="intentionally_excluded",
            mandatory=False,
            scope="migration",
            mapping="The pre-Mission task, task-link, and daily-operations schema vocabulary is intentionally not a Kepler v1 compatibility surface; the comparison remains as migration evidence.",
            probe_name="historical recursive schema comparison",
            passed=True,
            evidence={"comparison_passed": schema_pass, "comparison": schema_details},
        )
    )
    ...
    records.append(
        surface(
            "legacy_task_workflow_semantics",
            status="intentionally_excluded",
            mandatory=False,
            scope="migration",
            mapping="Kepler v1 removes the generalized task workflow lifecycle instead of recreating Mission or Operation behavior under new names.",
            probe_name="historical workflow comparison",
            passed=True,
            evidence={"comparison_passed": workflow_pass, "comparison": workflow_details},
        )
    )
    ...
    records.append(
        surface(
            "legacy_task_cli_behavior",
            status="intentionally_excluded",
            mandatory=False,
            scope="migration",
            mapping="Legacy task lifecycle commands are a deliberate clean break; repository routing and diagnostic commands remain, while Kepler v1 adds explicit ArchitectureMap, Plan, dispatch, result, and memory commands.",
            probe_name="historical command comparison",
            passed=True,
            evidence={
                ...
                "comparison_passed": commands_pass,
            },
        )
    )
    ...
    records.append(
        surface(
            "kepler_v1_cli_behavior",
            status="added",
            mandatory=True,
            scope="local",
            mapping="Kepler v1 exposes the six /kepler product actions through skills and deterministic ArchitectureMap, Plan, dispatch, WorkerResult, and memory helpers through the generated control project.",
            probe_name="executed Kepler v1 command-prefix contract",
            passed=kepler_commands.issubset(candidate_commands),
            evidence={
                "required": sorted(kepler_commands),
                "candidate_commands": sorted(candidate_commands),
                "missing": sorted(kepler_commands - candidate_commands),
            },
        )
    )

    with tempfile.TemporaryDirectory(
        prefix="kepler-source-test-snapshot-"
    ...
    records.append(
        surface(
            "legacy_task_lifecycle_behavior",
            status="intentionally_excluded",
            mandatory=False,
            scope="migration",
            mapping="The legacy task transition engine is intentionally excluded; Kepler persists only lightweight revisioned Plan agreement, readiness, receipts, and results.",
            probe_name="historical and candidate Ruby unit-suite comparison",
            passed=True,
            evidence={
    ...
    structured_validation = run(
        ["python3", str(setup_scripts / "validate_structured.py"), str(plugin.parent.parent)],
        cwd=plugin.parent.parent,
    )
    dispatch_contract_pass, dispatch_contract_evidence = static_contains(
        [
            plugin / "skills" / "kepler" / "SKILL.md",
            plugin / "skills" / "kepler" / "references" / "dispatch.md",
        ],
        {
    ...
    records.append(
        surface(
            "kepler_v1_coordination_contracts",
            status="added",
            mandatory=True,
            scope="local",
            ...
            passed=(
                ...
                and dispatch_contract_pass
            ),
            evidence={
                "required_schemas": sorted(v1_schemas),
                "missing_schemas": sorted(v1_schemas - candidate_schema_names),
                "candidate_tests": parse_test_summary(candidate_tests.stdout),
                "candidate_test_exit": candidate_tests.returncode,
                "structured_validation_exit": structured_validation.returncode,
                "dispatch_contract": dispatch_contract_evidence,
            },
        )
    )

    with tempfile.TemporaryDirectory(prefix="kepler-parity-") as acceptance_directory:
    ...
    source_doctor_codes = {
        ...
        if not code.startswith(("task.", "workflow."))
    }
    ...
    bulk_static_pass, bulk_static = static_contains(
        [
            plugin / "skills" / "kepler" / "SKILL.md",
            plugin / "skills" / "kepler-repo-bridge" / "SKILL.md",
            plugin / "skills" / "kepler-repo-bridge" / "references" / "configure-bridge-repos.md",
            candidate / "AGENTS.md",
    ...
    planning_pass, planning_evidence = static_contains(
        [
            plugin / "skills" / "kepler-plan" / "SKILL.md",
            plugin / "skills" / "kepler-plan" / "agents" / "openai.yaml",
            plugin / "skills" / "kepler-plan" / "references" / "planning-method.md",
            candidate / "AGENTS.md",
    ...
    review_pass, review_evidence = static_contains(
        [
            plugin / "skills" / "kepler-review" / "SKILL.md",
            plugin / "skills" / "kepler-review" / "agents" / "openai.yaml",
            plugin / "skills" / "kepler-review" / "references" / "review-method.md",
            candidate / "AGENTS.md",
    ...
    ci_pass, ci_evidence = static_contains(
        [
            plugin / "skills" / "kepler-ci" / "SKILL.md",
            plugin / "skills" / "kepler-ci" / "agents" / "openai.yaml",
            plugin / "skills" / "kepler-ci" / "references" / "delivery-method.md",
            candidate / "AGENTS.md",
    ...
    platform_pass, platform_evidence = static_contains(
        [
            plugin / "skills" / "kepler-platform" / "SKILL.md",
            plugin / "skills" / "kepler-platform" / "agents" / "openai.yaml",
            plugin / "skills" / "kepler-platform" / "references" / "platform-method.md",
            candidate / "AGENTS.md",
    ...
    database_pass, database_evidence = static_contains(
        [
            plugin / "skills" / "kepler-db" / "SKILL.md",
            plugin / "skills" / "kepler-db" / "agents" / "openai.yaml",
            plugin
            / "skills"
            / "kepler-db"
            / "references"
    ...
    upgrade_test = run(
        [
            ...
            "discover",
            "-s",
            str(plugin / "skills" / "kepler-upgrade" / "tests"),
            "-p",
            "test_*.py",
            "-v",
        ],
    ...
    upgrade_pass, upgrade_evidence = static_contains(
        [
            ...
            plugin / "skills" / "kepler-upgrade" / "SKILL.md",
            plugin / "skills" / "kepler-upgrade" / "agents" / "openai.yaml",
            plugin
            / "skills"
            / "kepler-upgrade"
            / "references"
    ...
    records.append(
        surface(
            ...
            mandatory=True,
            scope="local",
            mapping="Natural Kepler lifecycle intent maps to exact-version patch notes and a supported preservation-aware reinstall while generated Hubs and repositories remain protected user state.",
    ...
    artifact_paths = [
        plugin / "skills" / "kepler-artifacts" / "SKILL.md",
        plugin / "skills" / "kepler-artifacts" / "references" / "artifact-gates.md",
        plugin
        / "skills"
        / "kepler-artifacts"
        / "scripts"
        / "validate_deliverable.py",
        plugin / "skills" / "kepler-setup" / "references" / "setup-runbook.md",
    ...
    artifact_test = run(
        [
            ...
            "discover",
            "-s",
            str(plugin / "skills" / "kepler-artifacts" / "tests"),
            "-p",
            "test_*.py",
            "-v",
        ],
    ...
    stig_test = run(
        ["python3", "-m", "unittest", "discover", "-s", str(plugin / "skills" / "kepler-stig" / "tests"), "-p", "test_*.py", "-v"],
        cwd=plugin.parent.parent,
    )
    stig_files = {
        path.name
        for path in (plugin / "skills" / "kepler-stig" / "references").glob("*.md")
    }
    stig_scripts = {
        path.name for path in (plugin / "skills" / "kepler-stig" / "scripts").glob("*.py")
    }
    ...
    stig_method_pass, stig_method_evidence = static_contains(
        [
            plugin / "skills" / "kepler-stig" / "SKILL.md",
            plugin / "skills" / "kepler-stig" / "agents" / "openai.yaml",
            plugin / "skills" / "kepler-stig" / "references" / "evidence-contract.md",
            plugin / "skills" / "kepler-stig" / "scripts" / "evaluation_validator.py",
            candidate / "AGENTS.md",
    ...
    if args.source_stig_skill:
        ...
        candidate_attributes = literal_mapping(
            plugin / "skills" / "kepler-stig" / "scripts" / "ckl_parser.py",
            "ATTRIBUTE_MAP",
        )
        missing_attributes = {
            key: value
        ...
        stig_anchor_pass, stig_anchors = static_contains(
            [
                plugin / "skills" / "kepler-stig" / "SKILL.md",
                *sorted(
                    (plugin / "skills" / "kepler-stig" / "references").glob("*.md")
                ),
                *sorted(
                    (plugin / "skills" / "kepler-stig" / "scripts").glob("*.py")
                ),
            ],
            {
                "statuses": (
    ...
    records.append(
        surface(
            "legacy_reusable_method_documentation",
            status="intentionally_excluded",
            mandatory=False,
            scope="migration",
            mapping="Reusable engineering methods remain, while the removed control-plane design history and operations guide are retained only as comparison evidence rather than shipped Kepler methods.",
            probe_name="historical per-document normalized heading mapping",
            passed=True,
            evidence={"comparison_passed": docs_pass, "comparison": docs_evidence},
        )
    )

    repository_root = plugin.parent.parent
    required_v1_docs = {
        "README.md",
        "CHANGELOG.md",
        ...
        "docs/contracts/context-pack.md",
        "docs/contracts/worker-result.md",
        "docs/memory.md",
        "docs/upgrade.md",
        "docs/troubleshooting.md",
        "docs/migration-from-flightdeck.md",
    }
    missing_v1_docs = sorted(
        path for path in required_v1_docs if not (repository_root / path).is_file()
    )
    records.append(
        surface(
            "kepler_v1_product_documentation",
            status="added",
            mandatory=True,
            scope="local",
            mapping="Kepler v1 ships executable product, command, contract, memory, upgrade, troubleshooting, and clean-break migration documentation.",
            probe_name="required Kepler v1 documentation inventory",
            passed=not missing_v1_docs,
            evidence={"required": sorted(required_v1_docs), "missing": missing_v1_docs},
        )
    )

    source_inventory_output = (
        args.json.resolve().parent / "process-inventory-source-migration.json"
    )
    require_ignored_output(source_inventory_output)
    source_inventory_output.parent.mkdir(parents=True, exist_ok=True)
    source_inventory_output.unlink(missing_ok=True)
    source_inventory_result = run(
        [
            "python3",
            ...
            str(plugin),
            "--json",
            str(source_inventory_output),
        ],
        cwd=plugin.parent.parent,
    )
    try:
        source_inventory_report = json.loads(
            source_inventory_output.read_text(encoding="utf-8")
        )
    except (OSError, json.JSONDecodeError) as error:
        source_inventory_report = {
            "ok": False,
            "failures": [f"source migration inventory is unreadable: {error}"],
        }
    if not isinstance(source_inventory_report, dict):
        source_inventory_report = {
            "ok": False,
            "failures": ["source migration inventory must be a JSON object"],
        }
    source_inventory_failures = source_inventory_report.get(
        "failures",
        source_inventory_report.get("unresolved", []),
    )
    if not isinstance(source_inventory_failures, list):
        source_inventory_failures = [str(source_inventory_failures)]
    records.append(
        surface(
            "legacy_source_process_inventory",
            status="intentionally_excluded",
            mandatory=False,
            scope="migration",
            mapping="The source-backed inventory records removed legacy lifecycle and design-history paths as migration evidence; it is not the v1 distribution completeness gate.",
            probe_name="historical source migration inventory",
            passed=True,
            evidence={
                "output": str(source_inventory_output),
                "exit": source_inventory_result.returncode,
                "counts": source_inventory_report.get("counts", {}),
                "failures": source_inventory_failures,
            },
        )
    )
    ...
    try:
        ...
        process_inventory_report = {
            "ok": False,
            "failures": [f"distribution inventory is unreadable: {error}"],
        }
    process_inventory_pass = (
        process_inventory_result.returncode == 0
        and isinstance(process_inventory_report, dict)
        and process_inventory_report.get("ok") is True
    )
    records.append(
        surface(
            "strict_distribution_process_inventory",
            status="matched",
            mandatory=True,
            scope="local",
            mapping="Every shipped generated-control-project and plugin process surface has one neutral classification and all required mapped paths are present.",
            probe_name="strict candidate and plugin process inventory",
            ...
            evidence={
                ...
                "failures": process_inventory_report.get("failures", []),
            },
        )
    )
    ...
    runtime_evidence: dict[str, Any] = {
        "status": "not_run",
        "required_procedure": "kepler-setup/references/installed-acceptance.md",
    ...
    if args.runtime_acceptance:
        ...
        required_runtime = REQUIRED_RUNTIME_ACCEPTANCE_NAMES
    ...
    upgrade_runtime_evidence: dict[str, Any] = {
        "status": "not_run",
        "required_procedure": "kepler-setup/references/installed-acceptance.md#plugin-upgrade-acceptance",
        "validation_failures": [
    ...
    if args.upgrade_acceptance:
        ...
        if (
            ...
            or not plugin_id.startswith("kepler@")
            or plugin_id == "kepler@"
        ):
            upgrade_failures.append(
                "plugin_id must identify kepler at one configured marketplace"
            )
    ...
    setup_connection_static_pass, setup_connection_static = static_contains(
        [
            plugin / "skills" / "kepler-setup" / "SKILL.md",
            plugin / "skills" / "kepler-setup" / "references" / "setup-runbook.md",
            candidate / "AGENTS.md",
            candidate / "README.md",
            candidate / "lib" / "kepler" / "cli.rb",
            candidate / "hub" / "schemas" / "repository-declarations.schema.json",
        ],
        {
    ...
    return {
        "schema_version": "kepler.parity/v4",
...
def write_summary(path: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        "# Kepler semantic parity summary",
        "",
