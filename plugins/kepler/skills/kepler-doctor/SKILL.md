---
name: kepler-doctor
description: Run and interpret read-only Kepler Doctor and structured status checks. Use for control-project integrity, ArchitectureMap and Plan validity, exact project/path drift, dispatch prerequisites, bridge integrity, repository state, automation policy, or compliance sidecar parity.
---
...
`kepler.command.status.v1`, matching the requested command. Run
`bin/kepler doctor --json` only when the required capability is available.
...
and warning counts, ArchitectureMap/Plan/receipt/result state, repository and
sidecar counts, dispatch capability, and prioritized actions.
